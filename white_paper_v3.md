...
**Abstract:** This paper proposes a social network system that can be implemented in a peer-to-peer network environment. It enables any user to freely publish information while controlling the number of information publishers within a limited range, eliminating the need for third-party certification. Digital signatures ensure the authenticity, integrity, and non-repudiation of information. However, without a third-party organization to verify and associate the digital signature with the user's identity, the system may become flooded with a large number of spam accounts, rendering account-based penalty mechanisms useless. To address this, we propose a new solution for building social network services based on a peer-to-peer architecture, helping users effectively screen information publishers without relying on third-party authentication. In this system, all messages establish an ordered relationship through mutual reference and determine their source through signature verification. The homologous total order message sequence represents an information publisher's identity, and all messages in the system can form one or more message sets with a partial order relationship. Information publishers are free to create new species or associate other identities with a certain species. Users construct species ranges based on obtained species identification information and further filter suspicious information publishers based on identification relationships. This process transforms the centralized services' unified verification and consistent user range into a scalable species-based identity range determined by the users themselves.
