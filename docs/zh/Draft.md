...
我们认为，每个人的每个行为决定都是符合其主观认为符合其自身利益的。在一个理想的社会环境中，每个人行为的对错和自身利益反馈不会有冲突，所以当做决定时，表现为其行为符合普遍的对错观点。

而如果在一个社会环境中，如果每个人的行为总是以利益未考虑，而非对错的时候，并非是每个人考虑时是否利于自己的下意识标准改变了。而是这个社会环境的异常造成了个体利益和对错的冲突。我们认为对错是具备时间加成的全体的利益，所以在这种情况下，可以认为冲突发生在时间的长期与当下之间，也可能发生在个体与集体之间。

同时我们也应该看到，这种社会环境的异常必然是短期的。
...
熵增理论会造成维度的塌陷，不断有尺寸的高纬度向低纬度变化，无尽头。人类的热寂可能是低纬度的开始。物质是能量的尺寸塌陷的后果，比如大爆炸理论就是我们的尺寸维度中，所有能量逐步塌陷到更低的维度。

能量形成质子或中子的过程就是塌陷的过程。这是一个待整理的想法。
...
我们感受的超光速都是膨胀的速度对我们来说而非有质量的物体对于场。
...
另一个问题，有限无解的类似吹气球的宇宙膨胀模型是否来自于质量对于宇宙的扭曲？
...
「开始创建PDU的时候，我设定了两个祖先，用两个预设值的账户来建立新的账户，如此一层层创建下去，但后续这中形式被放弃。变为自由创建，而是自认为同类的账户会组成社区」
...
**几何原本**是可行的，因为几何是知识的一部分，包含在知识，理性等更高的范畴当中。而哲学却不能被理性清晰地描述，因为其包含其他的内容，包括理智，语言等。所以只能用大量语言近似的描述。
...
如果说时间只是一种主观感受，为何生命又受这种感受制约？其实是受自然这个造物主的限制，就好比造物主建造了一个可承受100次撞击的门。当撞击到100次时门就坏了。而时间作为感受，就是你感知到门被撞了几次。
...
由于只有被测物体能接近光速。所以也适用于当下。但局限在于其忽视了当自身告诉时，各方向的感受影响不均，时间失效。
...
如同电影Matrix中的虚拟世界，其存在是为了提高人的生存时间同一个道理。我们的意识就是那个Matrix。智能中自由，希望等均是与Matrix同样的作用。如同在Matrix中的觉醒，当明了这些之后也有两种选择。
...
以去中心化的观点来看待宇宙，其未必是一个有限无界的空间。既像原来认为的不断被吹大的更高维度的气球表面。存在另一种解释，因为熵增带来的扩散，而使得宇宙中我们可感知的部分是均匀的。我们能确认的只是其在以光为标准的可感知范围内的膨胀。
...
「系统的认知已经被放弃，变为去中心化的角度看这个世界，即智能看到部分。而且一个不可与外界交互的系统也是不可想象的，保证不可交互本身也是一种交互的方式。」
...
时间及空间的尺度都是智能需要了解系统能量波动变化规律时，需要的描述基础，及那个基础的刻度。空间可被认为是某种可被感知的能量波动形式的波的空间范围为基础来定义。而时间是以完成一个波动的过程为基础来定义，或者被认为是空间尺度和光速共同定义。

速度的有限的根源在于智能自身即存在于系统当中。

个体集中个体之间的交互只有个体所创建的东西。比如A将一块蛋糕送给B，这个过程中其实包含了A将蛋糕移动位置到B手上，B将蛋糕拿住，但这些并非系统集中的交互，只是两个个体对于外接的交互。真正的交互是A将系统集中创建的所有权的概念，由A交给了B。个体集中添加和去掉个体，也只能由个体集中现有的个体完成，是现有个体对其的承认和否认。
...
思维的方式终会从能得到某物或者能达到某个目标成长为能够去亲自完成某事。是从注重结果到注重过程的改变。同步发生的改变通常还有从只是想做某事，到想做任何事都可以从当下就开始。这也是人区别于动物的标志，更注重社会性存在。对物质需求和精神需求的比例会随着人的成长逐渐偏移。
...
其实事物只是各种客观实在，是什么只取决于我们如何去看待它。用什么维度去看它。比如一本书，也可以被看成是许多纸。或者说一个看着不像人的生物就不是人。
...
因为当生物只作为动物性存在时，群体的规模和个体生存课的的资源有一个最优的极值，那个值通常就是此种生物的群体规模上限。

而因为人更注重于精神性的存在，越大规模的群体对于精神的存在越有利。所以这个利益会平衡个体作为动物性存在得到的资源的下降趋势，所以规模会更大。
...
好的定义是利于群体，「与是否利于本个体无关」，而坏的定义是不利于群体存在「其实也与是否有利于个体无关，只是记不利于群体也不利于个体的事，缺乏发生的动机」。或者说好的定义为将群体的存在优先于个体，而坏是将个体存在优先于群体存在。

好的个体在坏的群体「由主要实行坏行为个体组成的群体」中做好事受到的惩罚远大于坏的个体在好的群体中做坏事受到的惩罚。因为惩罚本身就是对个体而言，但好的个体在坏的群体中做好事，并没有足够的利于个体的利益去平衡。

囚徒困境无法改变，只能离开而进入到好的群体，或更坏。好的个体在坏的群体中受到的惩罚不是一次性的，而是持续的，结果就是持续性的无所得。
...
康德认为时间、空间是我们认识世界的方式。「为自己能走向了康德而骄傲」我认为是尺度。三维空间是因为我们能感受三维，只能感受三维，而非宇宙或甚至我们自身是三维的。康德认为对时间、空间的认知是我们天生而有的观念，而非后天的经验，这点我并不确认，不肯定也不否定。「康德应该是对的，但这并非我们为人的原因，很多动物也有同样天生的感知能力」
...
公开的信息才能构成持续的、稳定的、可预计的结果。而隐秘的信息只能带来临时的，不确定的结果。依赖于秘密暴露的情况，且这是必然的。
...
民主社会和封建制度并无本质不同。封建的制度不确定性更大，任期的不确定，朝代的更替是一个更真实也更困难的选举过程。而且在封建制度中，加入喝生物延续的本能以治愈统治者，他们总试图给自身后代一个更好，更安定的世界。此制度也利于统治者从小的特别培养。「此处仅是探究封建能够存在几千年的内在合理性」
