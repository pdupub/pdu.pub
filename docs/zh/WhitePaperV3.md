...
中心化社交服务平台为实现以上两个功能提供了极大的帮助。通过手机号、邮箱等身份认证方式，平台可有效的限制虚拟身份创建以减少冗余信息；利用推荐算法及用户之间的关联可提高使用者获取信息的效率。但同时平台自身对于信息传播的影响也逐渐严重，如限制用户权限，屏蔽言论，利用推荐机制扩大特定话题的传播范围。我们并不否定舆论的引导在某些特定事件上存在的积极意义，只想表达对于信息「非自然传播」的忧虑。
...
* 社区定义/Community: 用于创建新的社区并制定规则，比如邀请限制等。
* 社区邀请/Invitation: 用于邀请其他账户加入用户自身所属社区。
...
我们相信任何可持续运行的系统都应该包含两个要素，即**必要的共识规则**和**合理的利益驱动**。所有参与者对于规则的认同，是构成系统的基础，或者说任何系统都是由认同某些同样规则的个体所组成。而合理的利益驱动则是系统参与者行为的原因，其不会改变，规则只有符合参与者的利益系统才能持续。以上两点并非特指去中心化的系统。通过定义**消息**、**账户**和**社区**的概念，我们已经为构建去中心化的信息传播系统提供了足够的规则。本节中我们进一步解释，为何基于这些规则，系统能够符合参与者的利益以使得系统持续的运行。
